...
<head>
    ...
    <title>Edit Properti - SIKOSUB</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* PENYESUAIAN CSS NAVBAR KIRI & LOGO */
        ...
        /* PENAMBAHAN CSS NAVBAR KANAN (PROFIL & DROPDOWN) */
        ...
        .btn-save { padding: 14px 40px; background: var(--success); border: none; border-radius: 12px; font-weight: 700; color: white; cursor: pointer; transition: 0.3s; font-size: 14px; box-shadow: 0 4px 15px rgba(16, 185, 129, 0.3); display: inline-flex; align-items: center; gap: 8px; }
...
<body>
    ...
    <nav class="topbar">
        <div class="topbar-left">
            ...
            <a href="{{ route('mitra.kost.index') }}" class="btn-kembali"><i class="fa-solid fa-arrow-left"></i> Batal & Kembali</a>
        </div>
    ...
    <div class="container">
        <div class="page-header">
            <h1 class="page-title">Edit Data Properti</h1>
            <p class="page-subtitle">Perbarui informasi, harga, dan ketersediaan kamar kost Anda</p>
        </div>

        <form action="{{ route('mitra.kost.update', $kost->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="admin-card">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label>Status Ketersediaan Kamar</label>
                        <select name="info_kamar" class="form-control">
                            <option value="Tersedia" {{ strtolower($kost->info_kamar) == 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                            <option value="Penuh" {{ strtolower($kost->info_kamar) == 'penuh' ? 'selected' : '' }}>Penuh</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-grid" style="grid-template-columns: 2fr 1fr;">
                    ...
                    <div class="form-group">
                        <label>Ganti Gambar Utama (Kosongkan jika tetap)</label>
                        <input type="file" name="gambar_utama" class="form-control" accept="image/*">
                        @if($kost->gambar_utama)
                            <span style="font-size:12px; color:var(--text-muted); margin-top:5px;">*Gambar saat ini: {{ $kost->gambar_utama }}</span>
                        @endif
                    </div>
                </div>
            </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="kategori_wilayah" class="form-control">
                            ...
                            <option value="unsub" {{ strtolower($kost->kategori_wilayah) == 'unsub' ? 'selected' : '' }}>Dekat Instansi Pendidikan (UNSUB)</option>
                            <option value="pemda" {{ strtolower($kost->kategori_wilayah) == 'pemda' ? 'selected' : '' }}>Dekat Instansi Pemerintah (Pemda)</option>
                            <option value="perusahaan" {{ strtolower($kost->kategori_wilayah) == 'perusahaan' ? 'selected' : '' }}>Dekat Instansi Perusahaan (Pabrik)</option>
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="no_wa" class="form-control" value="{{ $kost->no_wa ?? $kost->kontak_pemilik }}">
                    </div>
                </div>
            </div>
            ...
            <div class="admin-card">
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn-save"><i class="fa-solid fa-check-double"></i> Simpan Perubahan</button>
                </div>
            </div>
