@php
    ...
    // Mengambil data dari kolom yang tersedia di database
    $namaMitra = $activeUser ? ($activeUser->name ?? $activeUser->nama_pemilik ?? $activeUser->nama ?? 'Mitra') : 'Mitra';
    $emailMitra = $activeUser ? ($activeUser->email ?? '') : '';
    $phoneMitra = $activeUser ? ($activeUser->phone ?? '') : '';
@endphp
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Edit Profil Mitra - SIKOSUB</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .topbar-badge { background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 8px; padding: 4px 12px; color: var(--primary); font-size: 11px; font-weight: 800; margin-left: 5px; }
        ...
        .profile-avatar { width: 42px; height: 42px; border-radius: 50%; object-fit: cover; border: 2px solid var(--border); cursor: pointer; transition: 0.2s; }
        ...
        input { width: 100%; padding: 14px 16px; border: 1px solid var(--border); border-radius: 12px; font-family: 'Outfit', sans-serif; font-size: 14px; transition: 0.3s; }
        input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(26, 86, 219, 0.1); outline: none; }
        
        .btn-submit { background: var(--primary); color: white; width: 100%; padding: 14px; border: none; border-radius: 12px; font-weight: 700; cursor: pointer; transition: 0.3s; font-size: 15px; margin-top: 10px; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            <h2 style="margin-bottom: 25px;"><i class="fa-solid fa-user-pen" style="color: var(--primary);"></i> Edit Profil Mitra</h2>
            ...
            <form action="{{ route('mitra.profile.update') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{ old('name', $namaMitra) }}" required>
                </div>
                <div class="form-group">
                    <label>Email</label>
                    <input type="email" name="email" value="{{ old('email', $emailMitra) }}" required>
                </div>
                <div class="form-group">
                    <label>Nomor HP/WA</label>
                    <input type="text" name="phone" value="{{ old('phone', $phoneMitra) }}" required>
                </div>
